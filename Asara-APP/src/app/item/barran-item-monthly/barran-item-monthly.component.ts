...
@Component({
  selector: 'app-barran-item-monthly',
  templateUrl: './barran-item-monthly.component.html',
  styleUrls: ['./barran-item-monthly.component.scss']
})
export class BarranItemMonthlyComponent implements OnInit {
  ...
  getDayData(){
    if (this.formDay.valid){
      ...
      this.barrenService.getBarrenMonthly({
      ...
      this.barrenService.getExtraExpensesByMonth(date.toLocaleDateString()).subscribe(d => {
        this.exExpenses = d;
      }, e => {
        console.log(e);
      });
