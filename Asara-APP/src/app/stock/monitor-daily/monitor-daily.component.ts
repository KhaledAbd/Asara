...
@Component({
  selector: 'app-monitor-daily',
  templateUrl: './monitor-daily.component.html',
  styleUrls: ['./monitor-daily.component.scss']
})
export class MonitorDailyComponent implements OnInit {
  ...
  getDayData(){
    if (this.formDay.valid){
      this.monitorService.getMonitorDaily(this.formDay.get('day').value).subscribe(d => {
        this.rowData = d;
      }, e => {
        console.log(e);
      });
